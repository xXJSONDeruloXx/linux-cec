...
- `inactive_source_on_suspend`: Should cecd announce that it is no longer the active source when the device is
  suspended? Disabling this can prevent TVs that ignore the Standby command from switching to another input. Defaults
  to true.
